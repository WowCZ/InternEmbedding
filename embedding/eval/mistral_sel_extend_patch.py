...
def self_extend_forward(
    ...
    **kwargs,
    ...
    # EDIT by CZ
    key_position_ids = torch.cat([torch.arange(kv_seq_len, dtype=position_ids.dtype).unsqueeze(0) for _ in range(bsz)], dim=0)
    key_position_ids = key_position_ids.to(query_position_ids.device)
