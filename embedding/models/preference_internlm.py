...
class InternLMPrefModel(BaseEmbedder):
    def __init__(self, 
                 backbone: str, 
                 InternLMBackboneWrapper: BaseBackboneWrapper=InternLMBackboneWrapper, 
                 pool_type: str='cls', 
                 checkpoint_batch_size=-1, 
                 cls_dim: int= 1, # cls_dim
                 which_layer: int=-1, 
                 lora_config: bool=True, 
                 mytryoshka_indexes: list=None, 
                 normalize: bool = False):
        super().__init__(backbone, InternLMBackboneWrapper, pool_type, checkpoint_batch_size, cls_dim, which_layer, lora_config, mytryoshka_indexes, normalize)
